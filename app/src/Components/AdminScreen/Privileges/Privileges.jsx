...
const Privileges = () => {
  ...
  const handleAdd = async () => {
    try {
      // Fetch user ID based on selected username
      ...
      // Refresh users after adding privilege
      const usersResponse = await axiosPrivate.get('/users/privilages');
      setUsers(usersResponse.data);
      setFormData({ username: '', privilege_name: '' }); // Reset form data
    } catch (err) {
      setError(err.message);
    }
  };
  ...
  return (
    <div>
      ...
      <ul>
        {users.map(user => (
          <li key={user.id}>
            {user.name} - Privileges: {user.privileges.map(privilege => (
              <span key={privilege.id}>
                {privilege.name}
                <button onClick={() => handleRemove(user.public_id, privilege.name)}>Remove</button>
              </span>
            ))}
          </li>
        ))}
